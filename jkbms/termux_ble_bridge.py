...
async def main():
    if len(sys.argv) < 2:
        ...
        # Filter ketat: Hanya deteksi JK-/BMS dan abaikan sinyal beacon EZ-BT
        jk_devices = [d for d in devices if d.name and ("JK-" in d.name or "BMS" in d.name) and ("EZ" not in d.name)]
        if jk_devices:
